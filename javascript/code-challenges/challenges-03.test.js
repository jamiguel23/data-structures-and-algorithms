...
const containsAnd = (arr) => {
  return (
    arr.filter(and => and.includes('and'))
  );
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return (
    arr.filter(missNum => !forbiddenValues.includes(missNum))
  );
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  return arr.filter( stat => {
    // console.log('this is the stat', sat);
...
const getStatName = (arr, minBaseStat) => {
  ...
  let otherRes = result.map(stat => stat.stat.name);
...
const getCharactersWithoutChildren = (arr) => {
  ...
  let result = arr.filter(house => house.children == null)
...
const evenOddNumericValues = (arr) => {
  ...
  let finalRes = result.map(num => num % 2 ? 'odd' : 'even' );
